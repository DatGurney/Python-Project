...
def insert_usernames_table(Logins, filename = "stack_database.db"):
    conn = SQL.connect(filename)
    c = conn.cursor()
...
def select_usernames_table(Passwords, filename = "test.db"):
    conn = SQL.connect(filename)
    c = conn.cursor()
    print(filename)

    #SQL_select = "SELECT usernames, password FROM usernames"
    print("above the loop")
    print(c.execute("SELECT * FROM usernames"))
...
if __name__ == "__main__":
    ...
    print("select usernames table")
    logins = select_usernames_table(Logins)
    print("LOGINS",logins)
    print("Done")
