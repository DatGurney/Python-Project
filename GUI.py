...
class main_window:
    ...
    def createWidgets(self):
        ...
        self.menu1_button2 = Button(self.menu, text="New Client", command = self.create_new_entry)
        ...
        self.menu2_button = Button(self.menu, text = "New Job", command = add_time)
        ...
        self.menu1_button3 = Button(self.menu, text="Delete Job", command = self.create_delete_entry)
...
class new_entry:
    ...
    def createWidgets(self):
        self.entry.title("New Job")
        self.phone_label = Label(self.entry, text="Job Description")
...
class delete_entry:
    ...
    def createWidgets(self):
        ...
        self.ok_button = Button(self.entry, text="Delete", command = self.print_entries, width = 10)
        self.ok_button.grid(row = 3, column = 1, sticky = "WE", padx = 2, pady = 3)

        self.list.insert(END)

        self.quit_button = Button(self.entry, text="Quit", command = self.quit, width = 10)
        self.quit_button.grid(row = 3, column=2, sticky = "WE", padx=2,pady=3)

    def print_entries(self):
        self.Selected = self.list.curselection()
        job = self.list.get(first=self.Selected)
        print(job[1],"THIS IS THE JOB")
        CDB.delete_entry(job[1])
        self.entry.destroy()
        main_window()
...
class show_customer:
    ...
    def createWidgets(self):
        self.customer.title("Show Jobs")
        self.customer.geometry("600x400")

        self.list = Listbox(height = 10, width = 50, selectmode = EXTENDED)
        self.list.grid(row = 0, column = 0)
        ...
        self.button = Button(self.customer, text = "Search",command = self.show_user)
        self.button.grid(row = 2, column = 2)

        self.button2 = Button(self.customer, text = "option",command = self.get_menu_option)
        self.button2.grid(row = 5, column = 2)

        self.var = StringVar(self.customer)
        self.var.set("Choose Customer")
        self.option = OptionMenu(self.customer,self.var,"John", "Harry", "Dom")
        self.option.grid(row = 4, column = 0)

    def show_user(self, name = ""):
        self.list.delete(0, END)

        if (name == ""):
            self.List = CDB.customer_jobs(self.client_entry.get())
        else:
            self.List = CDB.customer_jobs(name)
